...
- [x] Notebook runs end-to-end: `cd /Users/ariasulin/Git/dwsTables && source .venv/bin/activate && jupyter nbconvert --execute notebooks/04_estimation_variance.ipynb --to notebook --output-dir=notebooks/`
- [x] `eda/estimation_variance.json` exists and is valid JSON: `python -c "import json; d=json.load(open('eda/estimation_variance.json')); print(f'Records: {len(d[\"modeling_df\"]):,}')"` — 14,042 records
- [x] `eda/estimation_variance.md` exists and is >500 chars — 2,762 chars
- [x] `eda/estimation_variance.png` exists and is >10KB — 449 KB
- [x] `scipy` is available (needed for `stats.linregress`): installed scipy 1.17.1 into venv
